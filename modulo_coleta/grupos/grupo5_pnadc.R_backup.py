...
def _localizar_rscript() -> str:
    ...
    Tenta em ordem:
    1. R dentro do ambiente conda ativo (CONDA_PREFIX)
    2. shutil.which() — funciona se R está no PATH do sistema
    3. Busca direta em C:/Program Files/R/ (Windows, instalação padrão)
    """
    import os

    # 1. Prioridade: R dentro do ambiente conda ativo
    conda_prefix = os.environ.get("CONDA_PREFIX")
    if conda_prefix:
        candidato_conda = Path(conda_prefix) / "Scripts" / "Rscript.exe"
        if candidato_conda.exists():
            logger.info("[Grupo 5] Rscript encontrado no conda: %s", candidato_conda)
            return str(candidato_conda)
    ...
    # 3. Fallback: instalação padrão Windows
    ...
    raise RuntimeError(
        ...
        "Para corrigir:\n"
        "  1. Instale o R via conda: conda install -c conda-forge r-base\n"
        "  2. Ou instale o R: https://cran.r-project.org/\n"
